...
fun solve(coefs: List<Int>, answer: Int): Pair<List<Int>, Pair<Double, Int>> {
    ...
    while (true) {
        ...
        for (i in 0 until populationSize / 2) {
            ...
            newPopulation[i*2] = mutate(children.first, Random.nextInt(0, populationSize - 1))
            newPopulation[i*2+1] = mutate(children.second, Random.nextInt(0, populationSize - 1))
        }
...
fun mutate(d: List<Int>, pos: Int) =
    d.mapIndexed{ index, it -> if (index == pos) it + Random.nextInt(-1, 1) else it }.toList()
